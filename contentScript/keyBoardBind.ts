...
const beforeEntryScene = () => {
  // 获得ap下限
  chrome.runtime.sendMessage({ message: 'get_ap_limit' }, (response) => {
...
const entryScene = () => {
  // 获得跳转地址
  chrome.runtime.sendMessage({ message: 'get_scene_href' }, (response) => {
    const { href } = response;
...
const bindKeyBoardListener = () => {
  // 获得键盘监听状态
  chrome.runtime.sendMessage({ message: 'get_key_board_listener_status' }, (response) => {
